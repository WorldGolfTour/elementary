{% macro get_existing_indexes(relation) %}
    ...
    {% set existing_indexes_table = elementary.run_query(
...
{% macro get_index_creation_query(table_nodes, existing_indexes) %}
    ...
    {% for table_node in table_nodes %}
        {% set relation = api.Relation.create(
            table_node.database, table_node.schema, table_node.alias
        ) %}
...
{% macro create_elementary_indexes() %}
    ...
    {% set table_nodes = (
        graph.nodes.values()
        | selectattr("package_name", "==", "elementary")
        | selectattr("config.materialized", "in", table_materializations)
    ) %}
    {% for table_node in table_nodes %}
        {% set relation = api.Relation.create(
            table_node.database, table_node.schema, table_node.alias
        ...
        {% set index_creation_query = elementary.get_index_creation_query(
